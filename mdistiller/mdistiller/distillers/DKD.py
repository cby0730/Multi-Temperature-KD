...
def mt_dkd_loss(logits_student, logits_teacher, target, alpha, beta, temperatures):
    gt_mask = _get_gt_mask(logits_student, target)
    other_mask = _get_other_mask(logits_student, target)
    ...
    for temperature in temperatures:
        ...
        all_loss.append(alpha * tckd_loss + beta * nckd_loss)
...
def mt_er_dkd_loss(logits_student, logits_teacher, target, alpha, beta, temperatures, t):
    gt_mask = _get_gt_mask(logits_student, target)
    other_mask = _get_other_mask(logits_student, target)
    ...
    for temperature in temperatures:
        ...
        all_loss.append(((alpha * tckd_loss + beta * nckd_loss) * entropy.unsqueeze(1)).mean())
...
class DKD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if self.er:
            if self.mt:
                loss_dkd = min(kwargs["epoch"] / self.warmup, 1.0) * mt_er_dkd_loss(
                    logits_student,
                    logits_teacher,
                    target,
                    self.alpha,
            ...
            if self.mt:
                loss_dkd = min(kwargs["epoch"] / self.warmup, 1.0) * mt_dkd_loss(
                    logits_student,
                    logits_teacher,
                    target,
                    self.alpha,
